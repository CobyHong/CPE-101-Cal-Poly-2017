...
def sphere_intersection_point(ray, sphere):
    ...
    if determinant < 0: #Means there are not roots
        ...
        root1 = ( (-b) + math.sqrt(determinant) ) / (2*a)#Just the Quadratic formula
        root2 = ( (-b) - math.sqrt(determinant) ) / (2*a)
        if (root1 >= 0 and root2 >= 0): #Take the smallest root and turn into point of intersection
            if root1>root2:
                return translate_point(ray.pt, scale_vector(ray.dir,root2))
            else: #root1<root2
                return translate_point(ray.pt, scale_vector(ray.dir,root1))
        elif root1 < 0 and root2 < 0 : #Now have to check if both negative
            return None
        elif root1 < 0 or root2 < 0: #If one of them is negative, return only the positive
            if root1 > 0:
                return translate_point(ray.pt, scale_vector(ray.dir,root1))
            elif root2 > 0:
                return translate_point(ray.pt, scale_vector(ray.dir,root2))
...
def find_intersection_points(sphere_list, ray):
    ...
    for i in range(len(sphere_list)):
        if sphere_intersection_point(ray, sphere_list[i]) != None: #Checks for roots in previous. And if there are roots.

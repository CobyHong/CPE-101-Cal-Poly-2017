...
def cast_ray(ray,sphere_list,ambient,light):
    ...
    if c == []:
        ...
        Pe = translate_point(c[nearest][1],scale_vector(sphere_normal_at_point(c[nearest][0],c[nearest][1]),0.01)) #Note carefully
        light_dir = normalize_vector(vector_from_to(Pe,light.pt)) #2
        visible = dot_vector(light_dir,sphere_normal_at_point(c[nearest][0],Pe)) #3
        ray_col = Ray(Pe,vector_from_to(Pe,light.pt)) #first bullet after 3 
